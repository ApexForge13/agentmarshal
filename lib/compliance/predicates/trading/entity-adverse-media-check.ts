...
// hit scoring → pass/review/fail) lands in Bubble 19. The proxy executes the BD
// call only when credentials are present (bdSerpSearch throws BdConfigError
// otherwise, caught inside the tool), so the evaluator stays hermetic in
// environments without a BD token — it then records a governance-only bd_call.
//
// Inputs:
