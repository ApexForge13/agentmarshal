...
import { bounceRateCompliantPredicate } from '../../../lib/compliance/predicates/operational/bounce-rate-compliant';
import {
  registerComposite,
  clearComposites,
  getComposite,
...
describe('operational bounce_rate_compliant predicate (Bubble 2 stub)', () => {
  beforeEach(() => {
    clearComposites();
  ...
  it('registers under the expected name', () => {
    const predicate = getComposite('bounce_rate_compliant');
    expect(predicate).toBeDefined();
    expect(predicate?.name).toBe('bounce_rate_compliant');
  });

  it('returns stub shape with InboxAllocator-metrics deferred_to anchor', async () => {
    const result = await bounceRateCompliantPredicate.evaluate(
      { sender_id: 'ses-002' },
      makeCtx(),
    );
    expect(result.result).toBe('stub');
    expect(result.predicate).toBe('bounce_rate_compliant');
    expect(result.reason).toMatch(/not yet implemented/i);
    expect(result.details.sender_id).toBe('ses-002');
  ...
  it('isAllowable returns false when this stub is in the trace', () => {
    const evals: CompositePredicateEvaluation[] = [
      { predicate: 'bounce_rate_compliant', result: 'stub', reason: '', details: {} },
    ];
