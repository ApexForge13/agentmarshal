import { describe, it, expect, beforeEach } from 'vitest';
import { senderReputationAboveThresholdPredicate } from '../../../lib/compliance/predicates/operational/sender-reputation-above-threshold';
import {
  registerComposite,
  clearComposites,
  getComposite,
  isAllowable,
  type CompositePredicateEvaluation,
} from '../../../lib/authzen/composite-dispatch';
import { NULL_EMITTER, type EvalContext } from '../../../lib/authzen/eval-context';

function makeCtx(): EvalContext {
  return {
    now: new Date('2026-05-22T14:00:00Z'),
    tenant_id: 't',
    agent_id: 'a',
    request_id: 'r',
    audit: NULL_EMITTER,
  };
}

describe('operational sender_reputation_above_threshold predicate (Bubble 2 stub)', () => {
  beforeEach(() => {
    clearComposites();
    registerComposite(senderReputationAboveThresholdPredicate);
  });

  it('registers under the expected name', () => {
    const predicate = getComposite('sender_reputation_above_threshold');
    expect(predicate).toBeDefined();
    expect(predicate?.name).toBe('sender_reputation_above_threshold');
  });

  it('returns stub shape with InboxAllocator-metrics deferred_to anchor', async () => {
    const result = await senderReputationAboveThresholdPredicate.evaluate(
      { sender_id: 'ses-001' },
      makeCtx(),
    );
    expect(result.result).toBe('stub');
    expect(result.predicate).toBe('sender_reputation_above_threshold');
    expect(result.reason).toMatch(/not yet implemented/i);
    expect(result.details.sender_id).toBe('ses-001');
    expect(result.details.deferred_to).toBe('InboxAllocator metrics integration');
  });

  it('isAllowable returns false when this stub is in the trace', () => {
    const evals: CompositePredicateEvaluation[] = [
      { predicate: 'sender_reputation_above_threshold', result: 'stub', reason: '', details: {} },
    ];
    expect(isAllowable(evals)).toBe(false);
  });
});
